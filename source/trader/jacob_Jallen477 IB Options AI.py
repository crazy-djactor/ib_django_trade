...
rf_rate = 0.017  # risk free rate, normally whatever 10 year treasury yield is
time_exp = 0.019  # (30.42 / 365.0) is roughly one month, (7/365) is roughly 1 week, 3/365 is roughly 3 days


expiration_premium_symbol_1_call = 1.023 # means strike price for the call is approx. 2.3% above current market price ,
...
num_contracts = 1 # specify number of contracts you want to trade
...
def Update():
    ...
    symbols = [symbol_1]  # NOTE: *** Enter whichever symbols from lines 65 - 94 that you actually want to test in here
    # INPUT DATA
    for symbol in symbols:
        try:
            ...
            # K - S
            theoretical_call_price = df['close'].values[-1:] * expiration_premium_symbol_1_call - df['close'].values[-1:]
            ...
            if predictions.values[-1:] == 1 and (current_ai_historical_accuracy > ai_pct_threshold_long) and (bsm_call_price > theoretical_call_price):
                ...
                contract = Option(symbol=symbol, lastTradeDateOrContractMonth=expiration_date_call, strike=strike_price_call,right='C', exchange='SMART')
                # place market order
            ...
            if predictions.values[-1:] == -1 and (current_ai_historical_accuracy < ai_pct_threshold_short) and (bsm_call_price < theoretical_call_price):
                ...
                contract = Option(symbol=symbol, lastTradeDateOrContractMonth=expiration_date_put, strike=strike_price_put,right='P', exchange='SMART')
                # place market order
                order = MarketOrder('BUY', num_contracts)
                trade = ib.placeOrder(contract, order)
                print(trade)
